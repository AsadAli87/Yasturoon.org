...
for i in range(0, 40, 10):  # 0, 10, 20, ..., 190
    ...
    if response.status_code == 200:
        ...
        for search_result_block in search_result_blocks[1:]:
            ...
            link_start = search_result_block.find('<a href="')
            link_end = search_result_block.find('"', link_start + 9)
            # Extract the link from the search result block
            link = search_result_block[link_start + 9:link_end]
            # Find the start and end indices of the description
            description_start = search_result_block.find('<span class="st">')
            description_end = search_result_block.find('</span>')
            # Extract the description from the search result block
            description = search_result_block[description_start + 18:description_end]
